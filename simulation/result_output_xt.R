...
library(reshape2)
set = 2; bw = ""
res <- readRDS(paste0("./onext", set, "_sim_nind_200", bw, ".rds"))
res$method <- gsub("FM", "FCART", toupper(res$method))
...
aeint <- function(beta, b, time) {
  naidx <- is.na(beta)
  res <- trapzRcpp(time[!naidx], abs((beta-b)[!naidx]))
  return(res)
}
seint <- function(beta, b, time) {
  naidx <- is.na(beta)
  res <- trapzRcpp(time[!naidx], ((beta-b)[!naidx])^2)
  return(res)
}
res2 <- res %>%
  group_by(method, id) %>%
...
df <- melt(res2, id = c("method", "id"), variable.name = "metric", value.name = "value") %>% 
  ...
  summarise(mean = round(mean(value), 3), sd= round(sd(value), 3),
            ...
            p75 = round(quantile(value, 0.75), 3)) %>% as.data.frame
...
for (m in unique(res30$method)) {
  ...
  ggsave(paste0("./", m,"_beta0_sim_xt", set, "_bands", bw, ".pdf"))
}
res31 <- res %>%
  group_by(method, id) %>%
...
#ggsave(paste0("./OneX/all5_beta1_sim_xt", set, "_bands", bw, ".pdf"), width = 6, height = 5)
for (m in unique(res31$method)) {
  ...
  ggsave(paste0("./", m,"_beta1_sim_xt", set, "_bands", bw, ".pdf"))
}
