## simulation for time-invariant case setting II
...
psi <- function(t, k) {
  if (k==1) {
   ...
   as.matrix((1/sqrt(5))*sin(2*pi*t/10))
  } else {
   as.matrix((1/sqrt(5))*cos(2*pi*t/10))
  }
}

# error variance structure: ind = TRUE for independent error, ind = FALSE for non-independent error
evar_fun <- function(tt, ind = TRUE) {
  if (ind) {
...
for (i in 1:200) {
  ...
  res <- rbind(res, resi)
  saveRDS(res, file = paste0("./onex2_sim_ind_", n, ".rds"))
